...
function notifyIos(userId, tokens, message, payload, callback) {
  ...
  feedback.on("feedback", function(devices) {
    monitorApnFeedback(userId, devices);
  });
...
function monitorApnFeedback(userId, feedbackDevices) {
  var feedbackTokens = feedbackDevices.map(function(d) { return d.token; });

  common.knex('devices').select('token', 'updated_at')
    .where({user_id: userId, platform: 'ios'})
    .whereIn('token', feedbackTokens)
    .then(function(storedDevices) {
      if (storedDevices.length < 1) { return; }
      storedDevices.forEach(function(device) {
        var feedbackItem = feedbackDevices.filter(function(d) { return d.device == device.token; })[0];

        if (typeof(feedbackItem) == "undefined") { return; }

        else if (new Date(device.updated_at) > new Date(feedbackItem.time)) { return; }

        else {
          if (process.env.NODE_ENV == 'test') {
            common.redis.rpush("yodel:unsubscribe", JSON.stringify({
              user_id: userId, token: device.token, platform: "ios"
            }));
          } else {
