...
function updateGcmNotificationKey(userId, registrationIds, isRemoval, callback) {
  var opts = {
    keyName: userId,
  ...
  redis.get('yodel:gcmusers:' + userId, function(err, data) {
    ...
    if (!err && data) {
      console.log("Found notification key: " + data);
    }

    var gcmConfig = common.config('gcm')
      , operator  = new gcm.NotificationKeyOperator(gcmConfig.project_number, gcmConfig.server_api_key);

    opts.key = data ? data : null;

    operator.requestUpdatedKey(opts, function(err, result) {
      console.log('Made it to the op callback');
