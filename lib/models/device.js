...
DeviceModel.notify = function(opts, callback) {
  if (!opts.user_id || isNaN(opts.user_id)) {
    ...
    knex.select()
      ...
      .then(function(results) {
        ...
        notifyIos(iosTokens, opts.message, opts.payload, function(err) {
          ...
          if (opts.include_notification_key) {
            ...
            .where({
              ...
              notifyAndroid(androidTokens, androidData, callback);
            }).catch(function(err) {
  ...
  function notifyIos(tokens, message, payload, callback) {
          ...
          if (err) {
            ...
            .then(function(inserts) {
              if (opts.sendNotificationKey) {
                notifyAndroid([opts.registrationId], {}, notificationKey, callback);
              } else {
                callback();
              }
            })
