...
DeviceModel.notify = function(opts, callback) {
  ...
  function notifyAndroid(devices, data, callback) {
    ...
    if (registrationIds.length < 1) {
      ...
      getGcmNotificationKey(opts.user_id, registrationIds, function(err) {
  ...
  function getGcmNotificationKey(userId, registrationIds, callback) {
    if (!userId) return callback("Invalid or missing option: user_id");
    if (!registrationIds || !registrationIds.length) {
      return callback("Invalid or missing option: registrationIds");
    }

    redis.get('yodel:gcmusers:' + userId, 0, function(err, data) {
      if (!err && data && data.length > 1) {
        console.log("Found notification key: " + data[1]);
      } else {
        console.log("Didn't find no damn notification key");
      }
    });

    var gcmConfig = common.config('gcm')
      , operator  = new gcm.NotificationKeyOperator(gcmConfig.project_id, gcmConfig.server_api_key)
      , operation = new gcm.NotificationKeyOperation();

    operation.operationType = 'create';
    operation.notificationKeyName = userId.toString();
    operation.registrationIds = registrationIds;
    ...
    operator.performOperation(operation, function(err, result) {
      ...
      if (result.notification_key) {
        ...
        console.log("Received Notification Key: " + result.notification_key);
        callback();
      }
    });
  }
