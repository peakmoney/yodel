...
function updateGcmNotificationKey(userId, registrationId, isSubscribe, callback) {
  ...
  redis.get('yodel:gcmusers:' + userId, function(err, data) {
    ...
    var gcmConfig    = common.config('gcm')
      , keyOperation = new gcm.NotificationKeyOperation(operationOpts)
      , manager      = new gcm.NotificationKeyManager(gcmConfig.project_number, gcmConfig.server_api_key);

    manager.performOperation(keyOperation, function(err, result) {
