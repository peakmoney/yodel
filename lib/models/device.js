          ...
          .then(function(inserts) {
            if (opts.platform === 'android') {
              updateGcmNotificationKey({userId: opts.user_id, token: opts.token
                , action: 'subscribe', dataNotificationKey: opts.send_notification_key}, function(err) {
...
DeviceModel.notify = function(opts, callback) {
        ...
        notifyIos(results, opts.message, opts.payload, function(err) {
          ...
          if (opts.include_notification_key) {
            ...
            .where({
              ...
              if (userResult && userResult.notification_key) {
                androidData.notification_key = userResult.notification_key;
              }
              notifyAndroid(androidDevices, androidData, callback);
            }).catch(function(err) {
  ...
  function notifyAndroid(registrationIds, data, callback) {
  ...
  knex.select().from('gcm_notification_users')
    ...
    .then(function(results) {
      if (!results || results.length === 0) {
        ...
        var notificationKeyName = 'user_' + opts.user_id + '_' + randomstring.generate(10)
          , operationOpts = {
        ...
        performOperation(operationOpts, function(err, result) {
          if (err) {
            callback(err);
          } else {
        ...
        performOperation(operationOpts, function(err) {
          if (err) return callback(err);

          knex('gcm_notification_users')
            .where({ user_id: opts.user_id })
            .update({
              notification_key: results[0].notification_key;
            })
